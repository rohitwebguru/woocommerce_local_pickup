...
<table id="price_calculator" class="wc-measurement-price-calculator-price-table <?php echo sanitize_html_class( $product->get_type() . '_price_calculator' ) . ' ' . sanitize_html_class( $calculator_mode ); ?>">
	...
	<!-- Custom Added Table Row -->
	<!--<tr class="price-table-row user-total-amount">
		<td>
			User Total Area (sq. ft.)			</td>
		<td>
			<span class="wc-measurement-price-calculator-total-amount" id="user_total_area"></span>
		</td>
	</tr>
	<script>
        function calculate() {
            var myBox1 = document.getElementById('length_needed').value;
            var myBox2 = document.getElementById('width_needed').value;
            // var result = document.getElementById('user_total_area');
            var myResult = myBox1 * myBox2;
            jQuery('#user_total_area').html(myResult);
        }
	</script>-->
	<!-- Custom Added Table Row -->
	...
	<tr class="price-table-row total-user-area">
		<td>
			Total User Area (sq. ft.)
		</td>
		<td>
			<span id="user_actual" class="amount_user_area" data-unit="<?php echo esc_attr( $product_measurement->get_unit() ); ?>"><?php echo $product_measurement->get_value(); ?></span>
		</td>
	</tr>
