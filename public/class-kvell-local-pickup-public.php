...
class Kvell_Local_Pickup_Public {
    ...
    public function init_hooks(){
        ...
        add_action( 'woocommerce_checkout_shipping', array( &$this, 'local_pick_up_map'), 10 );
		...
		add_filter( 'woocommerce_order_shipping_to_display', array( $this, 'order_shipping_to_display'), 5, 3);        
    }
    ...
    function group_lpp_method_label($located, $template_name, $args, $template_path, $default_path ){
        if ( 'cart/cart-shipping.php' === $template_name && is_checkout()) {
            ?>
            <?php if(!isset($_SESSION['local_pickup_discount']) && false && $_SESSION['local_pickup_discount']==0): ?>
            <style>
    ...
    function checkout_user_location($fields){
        ...
        /**$fields['shipping']['kvell_delivery'] = array(
            'type'      =>  'radio',
        ...
        $fields['shipping']['pickup_location_addresss'] = array(
            'type'      =>  'select',
	...
	function local_pick_up_map( $checkout ) {
        ...
        if ( isset($_SESSION['delivery_option_new']) ){
            ...
            <p class="form-row form-row-wide validate-required" style="display:block" id="kvell_delivery_field" data-priority="9999">
                <label for="kvell_delivery_same"  class="">Delivery Option&nbsp;<abbr class="required" title="required">*</abbr></label>
                <span class="woocommerce-input-wrapper">
                    <input type="radio" class="input-radio" value="same" name="kvell_delivery" id="kvell_delivery_same" <?php if($default=='same') echo 'checked="checked"'; ?>  /><label for="kvell_delivery_same" class="radio">Same Address</label>
                    <input type="radio" class="input-radio" value="local_pickup" name="kvell_delivery" id="kvell_delivery_local_pickup" <?php if($default=='local_pickup') echo 'checked="checked"'; ?> /><label for="kvell_delivery_local_pickup" class="radio">Local Pickups.</label>
                    <input type="radio" class="input-radio" value="different_address" name="kvell_delivery" id="kvell_delivery_different_address" <?php if($default=='different_address') echo 'checked="checked"'; ?> /><label for="kvell_delivery_different_address" class="radio">Different address</label>
                </span>
            </p>
            <p class="form-row select2" id="pickup_location_addresss_field" data-priority="10000" style="display: none;">
                <label for="pickup_location_addresss" class="">Pick UP address&nbsp;<span class="optional">(optional)</span></label>
                <span class="woocommerce-input-wrapper">
                    <select name="pickup_location_addresss" id="pickup_location_addresss" class="select" data-placeholder="Select pickup address ..">
                        <?php foreach($this->get_pickup_formatted_address() as $key=>$value): ?>
    ...
    function location_script(){
        wp_enqueue_script( 'wc-picup-custom-js', KVELL_PUBLIC. 'js/custom-new.js');
        $data=array();
	...
	public function order_shipping_to_display($shipping, $order, $tax_display  ) {
        $delivery= $order->get_meta( '_kvell_delivery' );
        if($delivery=='same') $shipping="Billing Address";
		if($delivery=='local_pickup') $shipping="Local Pickup";
        if($delivery=='different_address') $shipping="Shipping Address";
        return $shipping;
    }    
